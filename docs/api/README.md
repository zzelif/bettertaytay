# BetterTaytay API Documentation

Welcome to the BetterTaytay API documentation. This system provides comprehensive documentation for all API endpoints available in the BetterTaytay municipal government portal.

## Overview

BetterTaytay exposes multiple API categories for different purposes:
...
1. **Login** - Redirect to GitHub OAuth
   ```bash
   curl https://bettertaytay.org/api/admin/auth/login
   ```
...
3. **Make Authenticated Request** - Include session cookie in requests
   ```bash
   curl https://bettertaytay.org/api/admin/documents \
     -H "Cookie: session=<your-session-cookie>"
   ```
...
```bash
# Get documents
curl https://bettertaytay.org/api/openlgu/documents?limit=10

# Get persons
curl https://bettertaytay.org/api/openlgu/persons?term=1

# Get terms
curl https://bettertaytay.org/api/openlgu/terms
```
...
|-----|-------------|----------------|
| [Weather API](./weather-api.md) | Current weather and 3-hour forecast for Taytay | None |
...
**View Audit Logs:**
```bash
curl "https://bettertaytay.org/api/admin/audit-logs?action=create_document&limit=10" \
  -H "Cookie: session=<your-session-cookie>"
```
...
- **Production:** `https://bettertaytay.pages.dev`, `https://bettertaytay.org`
...
All admin requests must originate from `https://bettertaytay.org`.
...
# Public API
response = requests.get('https://bettertaytay.org/api/weather')
...
response = session.get('https://bettertaytay.org/api/admin/documents')
...
# OpenLGU API
curl "https://bettertaytay.org/api/openlgu/documents?limit=5"
```
...
- **Issues:** [GitHub Issues](https://github.com/zzelif/bettertaytay/issues)
